...
def read_news(data_root_path):
    ...
    for path in ['MINDsmall_train', 'MINDsmall_dev']:
...
def load_news_entity(news,EntityId2Index,data_root_path):
    ...
    with open(os.path.join('../MINDsamll/docs.tsv'), encoding='utf-8') as f:
        lines = f.readlines()
    news_entity = {}
...
def read_train_clickhistory(news_index,data_root_path,filename):
    ...
    # np.save('H:\code of 2rd work\FUM-main\data\train_sessions.npy', sessions)
    return sessions
    ...
    for i in range(len(lines)):
        ...
        for imp in imps.split():
            ...
            if label == '1':
                ...
                neg.append(docid)
        sessions.append([true_click,pos,neg])
    # np.save('H:\code of 2rd work\FUM-main\data\test_sessions.npy', sessions)
    return sessions
...
def read_test_clickhistory_noclk(news_index, data_root_path, filename):
    ...
    with open(os.path.join(data_root_path, filename)) as f:
        lines = f.readlines()
    sessions = []
    for i in range(128):  # len(lines)=73152
        _,uid,eventime, click, imps = lines[i].strip().split('\t')
        if click == '':
            clicks = []
        else:
