...
def get_flaten_user_encoder(title_word_embedding_matrix, category_dict, subcategory_dict):
    ...
    user_vecs = Fastformer(3, 40)([user_vecs, user_vecs, user_vecs])

    # lstm = LSTM(120, dropout=0.2, recurrent_dropout=0, activation='tanh', recurrent_activation='sigmoid')(user_vecs)  # 注意LSTM层的参数是为了能够用上cuDNN的加速
    # user_vecs = Dense(30, activation='sigmoid')(lstm)
    user_vecs = Dropout(0.2)(user_vecs)
    # lstm

    user_vecs = Reshape((MAX_CLICK, MAX_TITLE + 2, 120))(user_vecs)  # shape=(?,50,32,120)

    # CNN
    user_cexs_CNN = tf.nn.conv2d(user_vecs, strides=(1,3,2,1), padding='VALID', name='CNN_1')
    ...
    print('user_cexs_CNN:', user_cexs_CNN)
    return Model(user_vecs_input, user_vec)

def create_model_all(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict, subcategory_dict):
    ...
    user_vecs_all = keras.layers.Concatenate(axis=-1)([user_vec1, user_vec2, user_entity_vecs])
...
def create_model_new(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict,subcategory_dict):
    ...
    all_in_one_model = create_model_all(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict, subcategory_dict)
...
def create_model_cg(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict,
                    subcategory_dict):
    news_encoder = get_news_encoder(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict,
                                    category_dict, subcategory_dict)
...
def create_model_fg(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict,
                    subcategory_dict):
    news_encoder = get_news_encoder(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict,
                                    category_dict, subcategory_dict)
